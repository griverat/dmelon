...
class ECindex:
    ...
    def compute_anomaly(
        self, clim_period={"time": slice("1979-01-01", "2019-12-30")}
    ):
    ...
    def get_pcs(self, corr_factor=[1, -1]):
        ...
        if corr_factor is None:
            corr_factor = [1, 1]
        corr_factor = xr.DataArray(
            np.array(corr_factor), coords=[("mode", [0, 1])]
        )
        ...
        self.anom_pcs = (
            self.solver.projectField(
                self.sst_anom.sel(lat=slice(-10, 10)), neofs=2
            )
            * corr_factor
            / clim_std
        )
        ...
        self.anom_smooth_pcs = self.xconvolve(
            self.anom_pcs, kernel, dim="time"
        )
